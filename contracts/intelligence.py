...
#
# A reusable, on-chain escrow primitive that lets any party submit a claim
# together with raw evidence, stores a keccak-256 hash (and optionally the full
# evidence) on-chain, and uses AI consensus to resolve the claim.
#
# SUBMIT → RESOLVE → READ path:
#   submit_claim(id, text, evidence, category) → stores claim + evidence hash
#   resolve_claim(id) → AI consensus assesses evidence, stores verdict + reasoning
#   get_claim(id) → returns full claim with verdict, confidence, reasoning
#   get_evidence(id) → returns evidence hash + verification
#
# EVIDENCE ASSESSMENT:
...
class Intelligence(gl.Contract):
    ...
    @gl.public.write
    def submit_claim(self, claim_id: str, text: str, evidence: str,
                     category: str) -> None:
